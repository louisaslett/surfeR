...
setcookie('id', $_COOKIE['id'], array('expires' =>  time()+3600, 'domain' => '.louisaslett.com', 'samesite' => 'Lax'));
$id = $_COOKIE['id'];
...
if(count(glob("/2/*-$id.R"))>0) {
  ...
  echo "Your code is executing! Running for $runtime ... $i";
  exit();
}
if(count(glob("/1/*-$id.R"))>0) {
  ...
  echo "Your code is in the queue!  Currently in position $pos ... $i";
  exit();
}
?>
